...
The problem is, we can't evaluate this expression due to the non-linearities and high dimensionality of both x- and z-space.
...
IS allows us to sample from a different probability distribution (*proposal distribution*) and use weighted samples, where weighting is with respect to the original (*nominal*) pdf, to evaluate expectations.
We can write
...
$$
This estimator [has been shown](https://arxiv.org/abs/1705.10306) to optimise the modified KL-divergence $$KL(Q \mid \mid P)$$, with $$Q$$ and $$P$$ defined as
$$
  Q = Q_\phi (\mathbf{z} \mid \mathbf{x}) = \frac{1}{K} \prod_{k=1}^K q_\phi ( \mathbf{z}^{(k)} \mid \mathbf{x} ), \tag{9}
$$

$$
  P = P_\theta (\mathbf{z} \mid \mathbf{x}) = \frac{1}{K} \sum_{k=1}^K
  \frac{
      Q_\phi (\mathbf{z} \mid \mathbf{x})
    }{
      q_\phi (\mathbf{z^{(k)}} \mid \mathbf{x})
...
While similar to the original distributions, $$Q$$ and $$P$$ allow small variations in $$q$$ and $$p$$ that we would not have expected.
