...
# Forge
While it is a very much work in progress, I would like to show you how to setup your project using `forge`.
...
The arguments and kw-args passed to `forge.load` are automatically forwrded to the `load` function in the config file.
Why would you load a config by giving its filepath? To make code maintanance easier!
...
def process_dataset(x):
  pass
  # this functuon should return a minibatch, somehow
...
But if you would like to run a different config, you can just execute `python experiment.py --data_config some/config/file/path.py` without touching experimental code.
All this is very lightweight, as config files can return anything and take any arguments you find necessary.


### Smart checkpoints
Given that we have very general and flexible config files, it should be possible to abstract away loading of the model.
For example, it would be great if once we specify which configs we want to use to train a model, it should be possible to load a model snapshot without explicitly pointing to the config files used to train it.
We can do it by storing config files with model snapshots.
It can greatly simplify model evaluation and deployment and increase reproducibility of our experiments.
How do we do it?
This feature requires a bit more setup than just using config files, but bear with me - it might be even more useful.

This checkpoint system depends on a structure of directories.
We first need to initialize it, which creates the following structure:
```python
results_dir
  |run_name
    |1
...
```
`results_dir` is the top-level folder containing model checkpoints.
It can contain checkpoints for a number of jobs, where each job has a folder `run_name`.
We might want to re-run a specific job, and for this reason every run we run a job with the same run, `forge` creates a folder, whose name is an integral number - the number of this run.
It starts at one and gets incremented every time we start a new run of the same job.
Instead of starting a new job, we can also resume the last job by passing a flag.
In this case, we do not create a new folder for it, but just use the highest-numbered folder.


First, we need to do some imports and define some flags.
```python
from os import path as osp
...
We can then parse them and initialize our checkpoint.
```python
...
That's all.
Now we just need to use `logdir` and `resume_checkpoint` to store any logs and model snapshots.
For example:
```python
...
# Closing thoughts
Even though experimental code exhibits very similar structure among experiments, there seem to be no tools to streamline experimentation process.
...
Hope you can take something from it for your purposes.
