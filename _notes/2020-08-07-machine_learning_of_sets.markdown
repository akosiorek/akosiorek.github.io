...
Let's unpack this a bit, starting from the end,
...
Any structure in $$\mathbf{x}, \mathbf{y}$$, or both introduces constraints, and a successful application of any algorithm to a particular problem does heavily depend on whether or not this algorithm takes the relevant constraints into account.
...
In natural language-related problems a typical constraint is that of causality: a token at position t can depend on any previous tokens at position 1:t-1, but it cannot depend on any future tokens[^languecausality].

In the above examples, the dependencies between points (e.g. autoregressive dependence in NLP) are clear from the context.
However, if a data point is not a vector, matrix, or a sequence of vectors but it is a **set of vectors**, then these dependencies become less clear.
In particular, elements in an input set resamble elements in a dataset (i.e. lack of order), but the important difference is that they are **not independent**, therefore breaking the i.i.d. assumption.
Accounting for this specific structure in inputs or outputs of an ML model leads to a family of set learning problems, which have been gaining increasingly more attention in the machine learning community, and I though it would be useful to delve into machine learning of sets.
...
Loss functions tend to discard information about order, usually by taking the mean over data examples.
We can create permutation-invariant functions by following a similar logic.

Examples in a minibatch are processed independently (which reflects their i.i.d. nature), but if each entry in the minibatch contains more than just a single data point (many pixels in an image, points in a point cloud, tokens in a language sentence), then flattenting these points into a vector and feeding it into an MLP or a CNN results in different parameters being used for processing different data points, and hence order is used implicitly; feeding the points into an RNN reuses parameters, but introduces an explicit dependence on the order.

One simple solution to this issue is to treat points in a single example in the same way we treat examples in the minibatch: treat them independently.
...
Another factor contributing to the fact that the set-to-vector problem is quite easy, is that pooling operations naturally work with variable-sized sets and there is nothing extra we have to do to handle sets of variable cardinality.
This is not the case in the following two problems, where we have to explicitly take the set size into account.
...
Learning permutation-equivariance can be induced by data augmentation, since it is easy to generate different permutations, but usually comes at a decreased performance and/or longer training times compared to truly permutation-equivariant methods[^data_augmentation].
...
Since we generally have no guarantee that the generated sets will obey any ordering (why should they?), we have to apply losses that are invariant to that ordering.
...
* Instead of finding a matching, we can find a lower bound on what the matched loss would be. A popular choice here is the [Chamfer loss]()[^chamfer], which computes $$\sum_{x \in X} \min_{y \in Y} l(x, y) + \sum_{y \in Y} \min_{x \in X} l(x, y)$$. That is, for every element in one set, it finds the element in the other set that results in the lowest pairwise loss. This loss does not work for multisets as elements can be repeated.
...
Fortunately, we know how to solve the set-to-vector problem with a permutation-invariant neural net, and shortly I am going to describe some permtuation-equivariant methods for generation.
This is exactly what we recently explored in [Stelzner et. al., "Generative Adversarial Set Transformers", ICML 2020 Object-Oriented Learning Workshop](https://oolworkshop.github.io/program/ool_32.html).

Conincidentally, sometimes we have to deal with a set of latent variables inside a model. For example in Attend-Infer-Repeat (AIR, [paper](), [blog]()), a set of object-centered latent variables was used to render an image.
...
Until recently, there was no canonical method able to predict variable-sized sets in a permution-equivariant manner.
For completness, note that a function g is equivariant to permutations $$\pi $$ if $$\forall \pi$$: $$\pi g(X) = g(\pi X)$$.
[Zhang et. al., "Deep Set Prediction Networks", NeurIPS 2019](https://arxiv.org/abs/1906.06565) used the well-known (but still pretty cool!) observation that the gradient of a permutation-invariant function (such as the DeepSet embedding) is permutation equivariant with respect to the input set[^invgrad].
Their introduced model, DSPN, uses a fixed initial set that is then adapted via a nested loop of gradient-descent on a learned loss function.
This loss function compares the currently-generated set and the conditionin, telling us how well the current set and the conditioning match.
DSPN achieved quite good results on point-cloud generation (but only MNIST) and showed a proof-of-concept results to object detection in images.
...
While a cool idea, the gradient iteration learnt by DSPN is a flow field (see [Fig. 1](#DSPN_flow)), and it necessarily requires many iterations to reach the final prediction.
...
Not too long ago, [Vaswani et. al. showed that we can replace RNNs with attention, causal masking and position embeddings](https://arxiv.org/abs/1706.03762).
...
While details differ, the main finding is that an inittial set (randomly-sampled or deterministic and learned) passed through several layers of attention leads to state-of-the art set generation.
...
While the above approaches definitely work for generating sets, they make no use of the well-known area of statistics concerned with modelling sets: point processes!
...
This is in contrast to some of the previously-describe methods; e.g. DSPN uses heuristics to determine set size, which does or does not work depending which loss function it is used with ([see our TSPN paper for details](https://arxiv.org/abs/2006.16841)).
...
Why is that?
Well, comparing sets of two and sets of three elements is a bit like comparing square meters m$$^2$$ and cubic meters m$$^3$$, or like comparing apples and organes.
...
To do that, we have to account for a) the number of possible permutations of each set, and b) the unit volume (in case of metric space and comparing m$$^2$$ and m$$^3$$, we need to figure out how big a meter m$$^1$$ really is).
...
I would be curious to see if it improves results, as the Vu et. al. paper seems to suggest.
...
While correct, this approach forces us to use a bottleneck in shape of a single vector.
Perhaps a better option is to encode a set to an intermediate set, perhaps of smaller cardinality, and use that smaller set as a conditioning when generating the output set.
There are many methods how this can be done, and I will only mention that we explored some such problems in [Lee et. al, "Set Transformer", ICML 2019](https://arxiv.org/abs/1810.00825), while leaving the details as an exercise for the reader.
...
#### Acknowledgements
I'd like to give huge thanks to Fabian Fuchs, Thomas Kipf, Hyunjik Kim, Yan Zhang, George Papamakarios, and Danilo Rezende for insightful and inspiring discussions about the machine learning of sets. I'd also like to thank Hyunjik Kim and Fabian Fuchs for feedback on the initial version of this post.

[^acn]: See [Graves et. al., "Associative Compression Networks for Representation Learning", arXiv 2018](https://arxiv.org/abs/1804.02476) for an example where dataset (or minibatch) items are modelled jointly, and the loss depends on the whole minibatch/dataset.
...
[^invgrad]: More generally, the gradient of any invariant function is itself an equivariant function as noted in [Papamakarios et. al., "Normalizing Flows for Probabilistic Modeling and Inference", arxiv 2019](https://arxiv.org/abs/1912.02762).
...
[^chamfer]: Strictly speaking, it would be a lower bound if divided by two. The most popular form of the Chamfer loss omits this division, though.