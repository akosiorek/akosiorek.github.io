...
Even if scene models were available, we would need to augment with combinations of different transformations, which would result in an absolutely enormous dataset.
...
By definition, parts exhibit less variety in their appearance and shape than full objects, and it should be easier to learn about them from raw pixels.
...
The caveat here is that we still need to learn a part detector---it needs to predict part poses (translation, rotation, scale), too.
This should be much simpler than learning a full-blown object detector with similar capabilities, though.

Since poses change with the location of an observer (or rather the coordinate system), then a detector that can correctly identify pose of a part produces a viewpoint equivariant part representation.
...
Previous versions of capsules solved this by refining the assignment of parts to objects (routing) iteratively. This proved to be inefficient in terms of both computation and memory, and made it impossible to scale to bigger images.
...
Original capsules are a form of a feed-forward neural network with a specific structure, and are trained for classification.
...
That is why posterior distributions are often much more complicated than the prior or likelihood terms.
...
<figure id='diamond_square'>
  ...
  <figcaption align='center'>
  <b>Fig 3:</b> Is it a square, or is it a rhombus?
  </figcaption>
</figure>

A potential disadvantage is that, even though the representations might be viewpoint equivariant in the sense that they explicitly encode object coordinates, the encoder itself is not viewpoint equivariant. That means that if the model sees an object from a very different point of view than it is used to, it may fail to recognise the object.
Interestingly, this seems to be in line with human perception, cf [Figure 3](#diamond_square): imagine a square, and tilt it by 45 degrees, look away, and look at it again. Can you see a square? Or does the shape resemble a diamond (rhombus)?

# How can we turn object prototypes into an image?
I hope I have managed to convince you that learning capsule-like representation is possible, and that it is a good idea.
Now, I am going to describe the decoding stack of our model, and you can think of it as a (determinisitc) generative model.
Next, we will follow with a description of an inference network, which provides the representations we are looking for from the start.

We define a _capsule_ as a specialised part of a model that describes a single entity.
In the following, we will have _object capsules_, which recognise objects from parts, and _part capsules_, which extract parts and poses from the image.
...
Therefore, for $$n^\mathrm{th}$$ part capsule, its activations consist of the probability $$a_{k,n}$$ that the part exists, and a $$6$$-dimensional pose $$\mathbf{x}_m$$, which represents the position and orientation of the given part in the image.
...
These matrices are encouraged to be constant, but we allow a weak dependence on the object features $$\mathbf{c}_k$$ to allow for deformations.
Since any part can belong to only one object, we gather predictions from all object capsules corresponding to the same part capsule and arrange them into a mixture, and we explain parts by independent Gaussian mixtures.
...
Having generated part poses, we can take parts, apply affine transformations parametrised by the corresponding poses, and assemble the transformed parts into an image ([Figure 4](#mnist_strokes)).
...
For MNIST, for example, a good template would be a stroke, like in [the famous paper from Lake et. al.](http://www.sciencemag.org/content/350/6266/1332.short).
...
That is, for every part we need $$6$$ parameters of an affine transformation (we work in two dimensions) and a probability of the part being present; we could also predict some additional parameters.
...
This extra feature map will serve as an attention mask: we normalise it via a softmax, multiply with the remaining 7 feature maps, and aggregate them spatially.
...
Every transformed part is treated as a Gaussian mixture component, and we train PCAE by maximising the log-likelihood under this mixture.
...
<figure>
  ...
  <figcaption align='center'>
  <b>Fig 6:</b> The Object Capsule Autoencoder (OCAE) tries to explain part poses as a sparse set of objects, where every present object predicts several parts. It automatically discovers structure in the data, whereby different object capsule specialize to different objects.
  </figcaption>
</figure>
...
[Previous capsules]() used an EM-based inference procedure to make parts vote for objects.
This way, each part could start by initially disagreeing and voting on different objects, but eventually the votes would converge on a set of only a few objects.
We can also see it as explaining a potentially large set of parts with a potentially very sparse set of objects.
...
Since part activations describe parts, not pixels, which can have arbitrary positions in the image and in that sense have no ordering, set-input neural networks seem to be a better choice than MLPs, say.
...
<figure>
  <img style="display: box; margin: auto" src="{{site.url}}/resources/scae/blocks_v4.svg" alt="SCAE"/>
  <figcaption align='center'>
  <b>Fig 7:</b> The Stacked Capsule Autoencoder (SCAE) is composed of a Part Capsule Autoencoder (PCAE) followed by an Object Capsule Autoencoder (OCAE). It can decompose image into its parts and group parts into objects.
  </figcaption>
</figure>

In summary, a Stacked Capsule Autoencoder is composed of:
- one CNN with attention-based pooling,
- one Set Transformer,
...
- $$M$$ constant part templates, one for each part capsule.
