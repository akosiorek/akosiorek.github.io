...
First, we predict several masks such that they sum to one for every pixel.
Second, we penalise the masks so that they cannot be all black or all white.
...
At the end, in addition to updating the encoder's parameters, we also update the parameters of the masking neural net by maximising the loss L with respect to $$\phi$$.
...
RFL minimises the distance between representations extracted from two views of the same image.
That distance is minimised when the encoders are invariant to the transformations applied to the source image.
Here is a simple example: if we use a colour image and a grayscale version of that same image, we will get a representation that encodes the content (e.g. objects), and even brightness, but not the hue.
...
The masking model is trying to make 2) more difficult, hence it learns to mask strongly-correlated groups of pixels, which often correspond to semantically-meaningful object parts, but not necessarily correspond to objects---as discussed.
