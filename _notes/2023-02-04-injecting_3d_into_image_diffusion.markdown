...
<!-- TODO: from AdamG: I think this paragraph needs work, it feels its of lower quality as compared to everything before -->
So you have a model that can inpaint and outpaint images; you can change prompts every time you do it, and you can even seed the missing regions with fragments of different pictures. 
<!-- TODO: from AdamG: not sure which story are we coming back to - might need to be more emphasized before -->
Coming back to our visual story: amazing as the capabilities of text-to-image diffusion models are, you cannot use it to create a coherent visual story. At least not out of the box.
...
<!-- From Hyunjik re above:
Related to this first point (but slightly different) is that I think there is a difficulty in getting NeRF in dreamfusion to get off the ground - at init the rendered images at different views will look nothing like the text, and even if you text condition with high guidance scale to reduce gradient variance, the diffusion model will probably denoise each view towards one similar image, rather than towards different views of the same scene. Once NeRF is off the ground, and renders something that looks roughly like the text at any viewpoint, dreamfusion training should be able to improve it but the first stage described earlier seems like another reason why dreamfusion is hard to get to work.
 --> 
<!-- TODO: From AdamG: This addresses a different problem and a different question than "Why does the simple version not work that well?" so something needs changing here
 -->
...
Next, I'd like to describe RealFusion and NerfDiff: two very different takes at extracting geomtry from a diffusion model but in such a way that that extracted geometry (NeRF) is consistent with a provided image.
